...
class Arco{
	...
	method colisionArco(){
		...
		if(jugador == jugador2){
			jugador1.hizoGol()
			game.schedule(1000, {jugador1.cambiarPosicion(game.at(6,9))})
		    game.schedule(1000, {jugador2.cambiarPosicion(game.center().right(1))})
	    }
	    else{
	    	jugador2.hizoGol()
            game.schedule(1000, {jugador2.cambiarPosicion(game.at(30,9))})
	        game.schedule(1000, {jugador1.cambiarPosicion(game.center().left(1))})
	    }
